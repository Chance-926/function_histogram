...
module cdf_lut_generator (
    ...
    // =====================================================================
    input  logic        i_frame_done,    // 触发信号：上一级直方图统计/清零已完成
    ...
    // 状态输出
    output logic        o_lut_ready      // 告诉下一级：LUT 已经全部算好，可以开始映射新一帧了
);
    ...
    // 6. 核心数据通路：流水线对齐打拍
    // =========================================================================
    logic [8:0] loop_cnt_d1;  // 延迟 1 拍的计数器，用于对齐 BRAM 读出的数据，主要防止第256位数据赋到0位

    always_ff @(posedge clk or negedge rst_n) begin
        if (!rst_n) loop_cnt_d1 <= '0;
        else        loop_cnt_d1 <= loop_cnt;
    end

    // =========================================================================
    // 7. 数据通路 1：累加计算 CDF 与寻找 CDF_min
    // =========================================================================
    ...
    // 乘法器计算与截断 (DSP 资源自动推断)
    logic [31:0] cur_cdf; //分子中变化的部分
    logic [63:0] lut_mult_res;  //分子中的变量 乘 处理好的系数后 的结果
    logic [7:0]  final_lut_val; //最终新灰度值
    ...
    always_ff @(posedge clk or negedge rst_n) begin
        if (!rst_n) begin
            ...
            if (loop_cnt_d1 < 9'd256) begin
                // 第一步：计算公式里的 (CDF[i] - CDF_min)
                // 防御性编程：防止前面的 CDF_min 提取出错导致减法下溢出
                if (cdf_rd_data > cdf_min) cur_cdf = cdf_rd_data - cdf_min;
                else                       cur_cdf = '0;

                // 第二步：执行乘法 (cur_cdf * scale_hw)
                // 综合器会自动把这个 '*' 映射为芯片内部的硬件乘加器模块 (DSP)
                lut_mult_res = cur_cdf * i_div_quotient;

                // 第三步：位移截断 (等效于 Python 里的除以 65536)
                // >> 16 在硬件里就是直接取高位，不消耗任何逻辑资源！
                // 并且我们需要将结果限制在 0~255 (8-bit) 之间
                if ((lut_mult_res >> 16) > 255) final_lut_val = 8'd255;
                else                            final_lut_val = lut_mult_res[23:16]; 
                
                // 第四步：写回内部的 LUT RAM (映射表构建完成！)
                lut_ram[loop_cnt_d1[7:0]] <= final_lut_val;//原灰度值映射到新灰度值
            end
        end
    end
    ...
    // 10. 全局状态输出
    // =========================================================================
    assign o_lut_ready = (state == S_DONE);
